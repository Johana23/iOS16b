...
class InsertRecordViewController: UIViewController {
    ...
    @IBAction func btnSaveClicked(sender: AnyObject)
    {
        if(txtName.text == "")
        {
            Util.invokeAlertMethod("", strBody: "Ingresa un nombre.", delegate: nil)
        }
        else if(txtMarks.text == "")
        {
            Util.invokeAlertMethod("", strBody: "Ingresa el numero de materias.", delegate: nil)
        }
        else
        {
            if(isEdit)
            {
                ...
                if isUpdated {
                    Util.invokeAlertMethod("", strBody: "Registro actualizado.", delegate: nil)
                } else {
                    Util.invokeAlertMethod("", strBody: "Error al actualizar registro.", delegate: nil)
                }
            }
            else
            {
                ...
                if isInserted {
                    Util.invokeAlertMethod("", strBody: "Registro insertado.", delegate: nil)
                } else {
                    Util.invokeAlertMethod("", strBody: "Error al insertar registro.", delegate: nil)
                }
            }
            self.navigationController?.popViewControllerAnimated(true)
        }
